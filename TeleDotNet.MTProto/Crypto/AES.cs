...
namespace TeleDotNet.MTProto.Crypto
{
    ...
    public class AES
    {
        ...
        public static byte[] EncryptIGE(byte[] originPlaintext, byte[] key, byte[] iv)
        {
            ...
            var blocksCount = originPlaintext.Length / 16;
            var ciphertext = new byte[originPlaintext.Length];

            var ciphertextBlock = new byte[16];
            var plaintextBlock = new byte[16];
            for (var blockIndex = 0; blockIndex < blocksCount; blockIndex++)
            {
                Array.Copy(originPlaintext, 16 * blockIndex, plaintextBlock, 0, 16);
                ...
                Array.Copy(originPlaintext, 16 * blockIndex, iv2, 0, 16);
    ...
    public class AesEngine
    {
        ...
        */
        ...
        public int ProcessBlock(byte[] input, int inOff, byte[] output, int outOff)
        {
            ...
            if (inOff + 32 / 2 > input.Length) throw new InvalidOperationException("input buffer too short");

            if (outOff + 32 / 2 > output.Length) throw new InvalidOperationException("output buffer too short");
