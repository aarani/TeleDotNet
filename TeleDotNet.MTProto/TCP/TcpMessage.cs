...
namespace TeleDotNet.MTProto.TCP
{
    public class TcpMessage
    {
        ...
        public static TcpMessage Decode(byte[] body)
        {
            ...
            if (body.Length < 12)
                throw new InvalidOperationException("Oops, wrong size of input packet");
            ...
            using (var memoryStream = new MemoryStream(body))
            {
                using (var binaryReader = new BinaryReader(memoryStream))
                {
                    ...
                    if (packetLength < 12)
                        throw new InvalidOperationException($"]nvalid packet length: {packetLength}");
                    ...
                    if (checksum != validChecksum) throw new InvalidOperationException("Invalid Checksum. Skip!");
