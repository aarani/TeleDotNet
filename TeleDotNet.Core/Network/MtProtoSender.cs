...
using Ionic.Zlib;
using TeleDotNet.Core.MTProto;
using TeleDotNet.Core.MTProto.Crypto;
using TeleDotNet.Core.Requests;
using TeleDotNet.Core.Utils;

namespace TeleDotNet.Core.Network
{
    public class MtProtoSender
    {
        ...
        public async Task Send(TeleDotNet.TL.TLMethod request)
        {
            // TODO: refactor
        ...
        public async Task Send(byte[] packet, TeleDotNet.TL.TLMethod request)
        {
        ...
        public async Task<byte[]> Receive(TeleDotNet.TL.TLMethod request)
        {
        ...
        private bool processMessage(ulong messageId, int sequence, BinaryReader messageReader, TeleDotNet.TL.TLMethod request)
        {
            // TODO: check salt
        ...
        private bool HandleGzipPacked(ulong messageId, int sequence, BinaryReader messageReader, TeleDotNet.TL.TLMethod request)
        {
            uint code = messageReader.ReadUInt32();
        ...
        private bool HandleRpcResult(ulong messageId, int sequence, BinaryReader messageReader, TeleDotNet.TL.TLMethod request)
        {
            uint code = messageReader.ReadUInt32();
        ...
        private bool HandleBadServerSalt(ulong messageId, int sequence, BinaryReader messageReader, TeleDotNet.TL.TLMethod request)
        {
            uint code = messageReader.ReadUInt32();
        ...
        private bool HandlePong(ulong messageId, int sequence, BinaryReader messageReader, TeleDotNet.TL.TLMethod request)
        {
            uint code = messageReader.ReadUInt32();
        ...
        private bool HandleContainer(ulong messageId, int sequence, BinaryReader messageReader, TeleDotNet.TL.TLMethod request)
        {
            uint code = messageReader.ReadUInt32();
    ...
    public class FloodException : Exception
    {
        ...
        internal FloodException(TimeSpan timeToWait)
            : base($"Flood prevention. Telegram now requires your program to do requests again only after {timeToWait.TotalSeconds} seconds have passed ({nameof(TimeToWait)} property)." +
                    " If you think the culprit of this problem may lie in TeleDotNet's implementation, open a Github issue please.")
        {
    ...
    internal abstract class DataCenterMigrationException : Exception
    {
        ...
        private const string REPORT_MESSAGE =
            " See: https://github.com/sochix/TeleDotNet#i-get-a-xxxmigrationexception-or-a-migrate_x-error";
